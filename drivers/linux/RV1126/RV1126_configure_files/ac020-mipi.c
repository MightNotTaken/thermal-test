...
#define DRIVER_NAME "ac020-mipi"
//80M (clk)* 2(double ) *2 (lan) /8
#define ac020_LINK_RATE (80 * 1000 * 1000)
#define ac020_PIXEL_RATE		(40 * 1000 * 1000)
...
static const char * const ac020_supply_names[] = {
	"dovdd",	/* Digital I/O power */
	"avdd",		/* Analog power */
	"dvdd",		/* Digital core power */
};

#define ac020_NUM_SUPPLIES ARRAY_SIZE(ac020_supply_names)
...
struct ac020 {
	...
	struct regulator_bulk_data supplies[ac020_NUM_SUPPLIES];
	struct mutex lock;
...
static void ac020_get_default_format(struct v4l2_mbus_framefmt *format,int index)
{
	if(width>>(index*16))ac020_framesizes[index].width=(width>>(index*16))&0xffff;
	if(height>>(index*16))ac020_framesizes[index].height=(height>>(index*16))&0xffff;
	format->width = ac020_framesizes[index].width;
	format->height = ac020_framesizes[index].height;
	format->colorspace = V4L2_COLORSPACE_SRGB;
	format->code = ac020_framesizes[index].code;
				 ...
				 struct v4l2_subdev_mbus_code_enum *code)
{
	...
	if (code->index >= 1)
		return -EINVAL;
	code->code = ac020_framesizes[ac020->module_index].code;
				   ...
				   struct v4l2_subdev_frame_size_enum *fse)
{
	...
	fse->code = ac020_framesizes[ac020->module_index].code;
	fse->min_width  = ac020_framesizes[ac020->module_index].width;
	fse->max_width  = fse->min_width;
	fse->max_height = ac020_framesizes[ac020->module_index].height;
			  ...
			  struct v4l2_subdev_format *fmt)
{
	...
	mutex_lock(&ac020->lock);
	ac020_get_default_format(&fmt->format,ac020->module_index);
	mutex_unlock(&ac020->lock);
	return 0;
}
...
{
	struct ac020 *ac020 = to_ac020(sd);
	ac020_framesizes[ac020->module_index].width = fmt->format.width;
	ac020_framesizes[ac020->module_index].height = fmt->format.height;
	ac020_get_default_format(&fmt->format,ac020->module_index);
...
static int ac020_s_stream(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		ac020->streaming = 1;
		start_regs[19]= type;
		start_regs[22]= ac020_framesizes[ac020->module_index].width&0xff;
		start_regs[23]= ac020_framesizes[ac020->module_index].width>>8;
		start_regs[24]= ac020_framesizes[ac020->module_index].height&0xff;
		start_regs[25]= ac020_framesizes[ac020->module_index].height>>8;
		//startpreview
...
static int ac020_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	ac020_get_default_format(format,ac020->module_index);
				       ...
				       struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	fie->width = ac020_framesizes[ac020->module_index].width;
	fie->height = ac020_framesizes[ac020->module_index].height;
	fie->interval = ac020_framesizes[ac020->module_index].max_fps;
	return 0;
}
				   ...
				   struct v4l2_subdev_frame_interval *fi)
{
	...
	fi->interval = ac020_framesizes[0].max_fps;
			...
			const struct i2c_device_id *id)
{
	...
	ac020->link_frequency =	v4l2_ctrl_new_int_menu(&ac020->ctrls, NULL,
		V4L2_CID_LINK_FREQ, 0, 0, link_freq_menu_items);
			
	ac020->pixel_rate = v4l2_ctrl_new_std(&ac020->ctrls, &ac020_ctrl_ops,
